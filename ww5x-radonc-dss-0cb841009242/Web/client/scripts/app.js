...
//two functions: display radar chart (call when page loads); update radar chart(call when dvh is changed_)
    ...
    for(var ioar=0; ioar < number_organs; ioar++) { //for each organ 
      for(var j=bin_size/2; j < cdvh_array[ioar][iplan].length; j += bin_size){ // for each dose level
        for(var iplan=0; iplan < number_plans ; iplan++) { //construct for each plan
            temp_array[iplan] = cdvh_array[organ_index][iplan][j][1]
          } 
          
          corr = getPearsonCorrelation(temp_array.flat(), selected_array.flat());
          
          // check if correlation is greater than the smallest value in max_correlations
          absolute_corr = max_correlations.map(Math.abs);
          console.log("absolute value: " + absolute_corr)
          current_min = Math.min.apply(Math,absolute_corr);
          console.log("min correlation: " + current_min)
          console.log(Math.abs(corr))
          if (Math.abs(corr) > current_min && ioar != organ_index)  {
            console.log("adding correlation: " + corr)
            var new_index = max_correlations.indexOf(current_min);
            max_correlations[new_index] = corr;
            var abs_change = Math.max.apply(Math,temp_array) - Math.min.apply(Math,temp_array); 
            axis_lengths[new_index] = abs_change;
            dose_labels[new_index] = j;
            organ_labels[new_index] = organs[ioar];
            axis_labels[new_index] = dose_labels[new_index] + " gy " + organ_labels[new_index]; 
          }
        }
      }

      selected_organ = [organs[organ_index]];
      first_axis = [selected_point + " gy " + selected_organ];
      axis_labels = first_axis.concat(axis_labels);

      selected_change = [Math.max.apply(Math, selected_array) - Math.min.apply(Math, selected_array)];
      axis_lengths = selected_change.concat(axis_lengths);
      update_radar(axis_labels,axis_lengths);
}

function update_radar(newlabels,newaxes){
  radarChart.data.datasets[0].data = newaxes;
  radarChart.data.labels = newlabels;
  radarChart.update();
}

function initialize_radar() {
